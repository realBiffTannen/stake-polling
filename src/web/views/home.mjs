 ...
 * live, the revenue model the roster reports for it (the 10% revenue share,
 * or the 5% GGR split across providers), its approval stage, and a link to
 * its page on the studio itself.
 *
...
export function renderHome(state) {
  ...
  const rateBySlug = new Map(rows.map((r) => [r.name, r.rate ?? null]));
