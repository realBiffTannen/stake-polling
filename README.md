...
| Route | What it shows |
|---|---|
| `/` | **Overview.** One row per live game (bets, turnover, studio P/L month-to-date, P/L today, online) with a total row; a *Games* table of every title in the catalogue with the star rating the Engine studio shows for it (out of three), whether it is live, its revenue model (the 10% revenue share or the 5% GGR split across providers, from the roster's rate), its approval stage and a link to its page on the studio; plus a *Not yet live* table of every catalogue title that is not turned on (status, approval stage, captured RTP / modes / max win). Simple figures only - every game name opens its game page. |
| `/games` | **Games.** The catalogue: every title, live or not, with its Engine star rating, status, revenue model, approval stage and a link to its page on the studio, its lifetime bets, players, turnover and studio P/L, and a ▸ that opens the row into this month's figures, the captured math and its bet modes; plus the titles not yet live with their captured math - the same two tables the Overview carries. Every table on both pages sorts by any column: click a heading, again to reverse. |
